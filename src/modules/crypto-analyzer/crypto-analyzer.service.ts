...
import { checkCMCRate, checkGeckoRate, notifyModelCMCMapper, notifyModelGeckoMapper } from './crypto-analyzer.helper';
...
export class CryptoAnalyzerService {
  ...
  async getCoinGeckoCryptos() {
    ...
    return data?.filter(checkGeckoRate).map(item => notifyModelGeckoMapper(item));
  }
  ...
  async getCMCCryptos() {
    ...
    return data?.data?.filter(checkCMCRate).map(item => notifyModelCMCMapper(item));
  }
